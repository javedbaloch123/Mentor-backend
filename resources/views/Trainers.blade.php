   ...
   <div class="row">
    ...
    <div class="col-lg-8 mx-auto">
		  @if (Session::has('success'))
                    ...
                    @endif
		<button class="btn btn-primary px-5" data-bs-toggle="modal" data-bs-target="#trainerModal">Add Trainer</button>
       <div class="card">
					<div class="card-body">
			                   ...
			                   @if ($trainers)
							    @foreach ($trainers as $trainer)
									 <tr role="row" class="odd">
										<td class="sorting_1">{{ $trainer->name }}</td>
										<td>{{ $trainer->desc }}</td>
										<td>{{ $trainer->skill }}</td>
										<td>{{ $trainer->picture }}</td>
								       <td><a href="{{ route('edit.trainer',$trainer->id) }}"><button class="btn btn-primary">Edit</button></a></td>
										<td><button class="btn btn-danger" onclick="Delete({{ $trainer->id }})">Delete</button></td>
									</tr>
								@endforeach   
							   @endif
...
<script>
	function Delete(id) {
            if (confirm('Sure You Want To Delete')) {
                $.ajax({
                    url: '{{ route("delete.trainer", ":id") }}'.replace(':id', id),
